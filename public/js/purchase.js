$(document).ready(function() {
    ...
    $('#supplier_id').select2({
        ...
        gettotaldamaged(data.id);
        set_supplier_address(data);
    });

    function gettotaldamaged(id){
        $.ajax({
            method:"post",
            url: "/contacts/gettotaldamaged", 
            data:{
                id:id
            },
            success: function(result){
                $("#supplier_damaged_div").html("Total damaged product:"+__currency_trans_from_en(result, false, true)+" qty");
        }});
    }
    ...
    $(document).on('click', '.remove_purchase_entry_row', function() {
        ...
        swal({
            ...
            if (value) {
                ...
                remove_from_session(product_id, variation_id);
            }
        });
    });

    function remove_from_session(product_id, variation_id){
        $.ajax({
            method: 'POST',
            url: base_path + '/purchases/remove-from-session',
            dataType: 'json',
            data: {
