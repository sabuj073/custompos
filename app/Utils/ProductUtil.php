...
class ProductUtil extends Util
{
     ...
     */
     ...
     public function decreaseDamagedProductQuantity($product_id, $variation_id, $location_id, $decrease_qty)
     {
         DB::transaction(function () use ($product_id, $variation_id, $location_id, $decrease_qty) {

             $damagedLines = StockDamagedLine::where('product_id', $product_id)
                 ->where('variation_id', $variation_id)
                 ->orderBy('created_at') // Process oldest entries first
                 ->get();
             ...
             foreach ($damagedLines as $line) {
                 ...
                 if ($line->quantity <= $decrease_qty) {
                     // Set the quantity to zero and reduce the remaining decrease_qty
                     $decrease_qty -= $line->quantity;
                     $line->quantity = 0;
                 } else {
                     // Reduce the quantity and set decrease_qty to zero
                     $line->quantity -= $decrease_qty;
                     $decrease_qty = 0;
                 }
     ...
     */
     ...
     public function getDetailsFromVariationDamaged($variation_id, $business_id, $location_id = null, $check_qty = true)
     {
        $query = Variation::join('products AS p', 'variations.product_id', '=', 'p.id')
        ->join('product_variations AS pv', 'variations.product_variation_id', '=', 'pv.id')
        ->leftjoin('stock_damaged_lines AS sdl', function ($join) {
            $join->on('variations.id', '=', 'sdl.variation_id')
    ...
    if ($check_qty) {
        // Use stock damaged lines to check for quantities
        $query->select(
            ...
            'variations.sub_sku',
            'p.barcode_type',
            DB::raw('IFNULL(SUM(sdl.quantity), 0) AS qty_available'), // Sum of quantities from stock damaged lines
        ...
        // If quantity check is not enabled, include stock damaged lines data but don't check quantities
        $query->select(
            DB::raw("IF(pv.is_dummy = 0, CONCAT(p.name, ' (', pv.name, ':', variations.name, ')'), p.name) AS product_name"),
            'p.id as product_id',
            'p.brand_id',
    ...
    public function filterProductdamaged($business_id, $search_term, $location_id = null, $not_for_selling = null, $price_group_id = null, $product_types = [], $search_fields = [], $check_qty = false, $search_type = 'like')
{
    $query = Product::join('variations', 'products.id', '=', 'variations.product_id')
                ->active()
                ->whereNull('variations.deleted_at')
                ->leftJoin('units as U', 'products.unit_id', '=', 'U.id')
                ->join('stock_damaged_lines as SDL', 'variations.id', '=', 'SDL.variation_id'); // Join with stock_damaged_lines

    // Optionally, filter products not for selling
    if (! is_null($not_for_selling)) {
        $query->where('products.not_for_selling', $not_for_selling);
    }

    // Include price group filter if necessary
    if (! empty($price_group_id)) {
        $query->leftJoin(
            'variation_group_prices AS VGP',
            function ($join) use ($price_group_id) {
    ...
    // Filter products by business ID and type
    $query->where('products.business_id', $business_id)
          ->where('products.type', '!=', 'modifier');
    ...
    if (! empty($search_term)) {
        ...
        if ($search_type == 'like') {
            $query->where(function ($query) use ($search_term, $search_fields) {
                ...
                // Additional custom fields search
                if (in_array('product_custom_field1', $search_fields)) {
                    $query->orWhere('product_custom_field1', 'like', '%'.$search_term.'%');
                }
                if (in_array('product_custom_field2', $search_fields)) {
    ...
    // Include check for quantity from stock_damaged_lines
    if ($check_qty) {
        $query->havingRaw('SUM(SDL.quantity) > 0'); // Ensure sum of quantity is greater than 0
    }
    ...
    $query->select(
            ...
            'variations.id as variation_id',
            'variations.name as variation',
            DB::raw('SUM(SDL.quantity) as qty_available'), // Sum the quantity from stock_damaged_lines
            'variations.sell_price_inc_tax as selling_price',
            'variations.sub_sku',
            'U.short_name as unit'
        );

    // Include variation group price if necessary
    if (! empty($price_group_id)) {
        $query->addSelect('VGP.price_inc_tax as variation_group_price');
    }

    // Lot-based search handling
    if (in_array('lot', $search_fields)) {
        $query->leftJoin('purchase_lines as pl', 'variations.id', '=', 'pl.variation_id')
              ->addSelect('pl.id as purchase_line_id', 'pl.lot_number');
    }

    // Group by variation ID to avoid duplicate rows
    $query->groupBy('variations.id', 'products.id', 'U.short_name', 'variations.name', 'variations.sell_price_inc_tax', 'variations.sub_sku');

    return $query->orderBy('qty_available', 'desc') // Sort by summed damaged quantity
                 ->get();
}
