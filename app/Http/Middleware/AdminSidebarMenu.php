...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($common_settings) {
                        ...
                        if (auth()->user()->can('purchase.update')) {
                            $sub->url(
                                action([\App\Http\Controllers\PurchaseFinalizeController::class, 'index']),
                                'Finalize Purchase',
                                ['icon' => 'fa fas fa-undo', 'active' => request()->segment(1) == 'purchases-finalize']
                            );
                        }
                        if (auth()->user()->can('purchase.update')) {
                            $sub->url(
            ...
            //damaged product dropdown
            if (in_array('stock_adjustment', $enabled_modules) && (auth()->user()->can('purchase.view') || auth()->user()->can('purchase.create'))) {
                $menu->dropdown(
                    __('stock_damaged.stock_adjustment'),
                    function ($sub)  use ($enabled_modules, $is_admin, $pos_settings)  {
                        if (auth()->user()->can('purchase.view')) {
                            $sub->url(
                                action([\App\Http\Controllers\DamagedProductController::class, 'index']),
                                __('stock_damaged.list'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'stock-adjustments' && request()->segment(2) == null]
                            );
                        }
                        if (auth()->user()->can('purchase.create')) {
                            $sub->url(
                                action([\App\Http\Controllers\DamagedProductController::class, 'create']),
                                __('stock_damaged.add'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'stock-adjustments' && request()->segment(2) == 'create']
                            );
                        }

                            $sub->url(
                                action([\App\Http\Controllers\SellController::class, 'selldamaged']),
                                __('Sell Damaged'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'sells' && request()->segment(2) == 'create' && empty(request()->get('status'))]
                            );
                        ...
                        if ($is_admin || auth()->user()->hasAnyPermission(['sell.view', 'sell.create', 'direct_sell.access', 'direct_sell.view', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping'])) {
                            $sub->url(
                                ...
                                __('lang_v1.all_sales'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'damaged-sells' && request()->segment(2) == null]
                            );
                        }
                    },
                    ['icon' => 'fa fas fa-database']
            ...
            // if (auth()->user()->can('all_expense.access') || auth()->user()->can('view_own_expense')) {
                $menu->dropdown(
                    __('expense.notification_alert'),
                    function ($sub) {
                        $sub->url(
                            action([\App\Http\Controllers\Notifications\StockAlertNotificationController::class, 'getProductStockAlert']),
                            __('lang_v1.stock_alert_notification'),
                            ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'stock' && request()->segment(2) == 'alert-notification']
                        );

                        
                            $sub->url(
                                action([\App\Http\Controllers\Notifications\PurchasePaymentDueController::class, 'getPurchasePaymentDues']),
                                __('lang_v1.purchases_alert_notification'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'purchases' && request()->segment(2) == 'alert-notification']
                            );

                            $sub->url(
                                action([\App\Http\Controllers\Notifications\SalesPaymentDueController::class, 'getSalesPaymentDues']),
                                __('lang_v1.sales_payment_due_alert'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'sales-payment-due' && request()->segment(2) == 'notification']
                            );
                    },
                    ['icon' => 'fa fas fa-minus-circle']
                )->order(45);
