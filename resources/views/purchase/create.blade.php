		...
		<div class="row">
			<div class="@if(!empty($default_purchase_status)) col-sm-4 @else col-sm-3 @endif">
				<div class="form-group">
					@php
						$supplier = DB::table('contacts')->find(request()->get('supplier_id'));
						$supplier_name = $supplier ? $supplier->name : '';
					@endphp
					{!! Form::label('supplier_id', __('purchase.supplier') . ':*') !!} <small style="color: blue;">{{ $supplier_name }}</small>
				...
				<div id="supplier_address_div"></div><br>
				<div id="supplier_damaged_div"></div>
			</div>
			<div class="@if(!empty($default_purchase_status)) col-sm-4 @else col-sm-3 @endif">
				<div class="form-group">
...
@section('javascript')
	...
	<script>
		$(document).ready(function(){
			@if(Session::has('stock_alert_products'))
				@foreach (Session::get('stock_alert_products') as $item)
					var data = "{{ $item }}";
					var values = JSON.parse(data.replace(/&quot;/g,'"'));
					// console.log(values[0]);
					get_purchase_entry_row(values[0], values[1]);
				@endforeach
			@endif
		});
	</script>
