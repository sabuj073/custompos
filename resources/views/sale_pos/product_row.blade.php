...
<tr class="product_row" data-row_index="{{ $row_count }}"
    ...
    <td>
        ...
        @if (($edit_price || $edit_discount) && empty($is_direct_sell))
            ...
            <div title="@lang('lang_v1.pos_edit_product_price_help')">
                <span class="text-link text-info cursor-pointer">
                    {!! $product_name !!} | Stock-{{ $product->qty_available }}
                    &nbsp;<i class="fa fa-info-circle"></i>
                </span>
            </div>
        @else
        ...
        <div class="input-group" style="display: flex; width: 85%; margin-top: 5px;">
            <input type="password" class="form-control" id="tooglepass{{ $product->product_id }}" readonly
                value="Purchase price {{ round(App\PurchaseLine::where('product_id', $product->product_id)->first()->purchase_price, 2) }}" /><button type="button"
                class="btn btn-secondary" onclick="ShowHidefl({{ $product->product_id }})">Show</button>
        </div>

        <script>
            function ShowHidefl(row) {
                var x = document.getElementById("tooglepass" + row);
                if (x.type === "password") {
                    x.type = "text";
                } else {
                    x.type = "password";
    ...
    @php
        $get_product = App\Product::find($product->product_id);
    @endphp


    <td class="text-center">
        <span
            class="">{{ $product->line_discount_amount ? round($unit_price_inc_tax + $product->line_discount_amount, 2) : round($unit_price_inc_tax, 2) }}</span>
    </td>
    <td class="text-center">
        <span
            class="pos_custom_updated_discount">{{ $product->line_discount_amount ? round($product->line_discount_amount, 2) : round($get_product->discount, 2) }}</span>
    </td>
    
    <td class="">
        <input type="text" readonly style="background: none !important; border: none !important; padding-top: 0px;" name="products[{{ $row_count }}][unit_price_inc_tax]"
            class="form-control pos_unit_price_inc_tax input_number" value="{{ @num_format($unit_price_inc_tax) }}"
            @if (!$edit_price) readonly @endif
            @if (!empty($pos_settings['enable_msp'])) data-rule-min-value="{{ $unit_price_inc_tax }}" data-msg-min-value="{{ __('lang_v1.minimum_selling_price_error_msg', ['price' => @num_format($unit_price_inc_tax)]) }}" @endif>
    </td>
