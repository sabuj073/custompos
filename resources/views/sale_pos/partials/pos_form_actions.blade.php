...
<div class="row">
	<div class="pos-form-actions">
		<div class="col-md-12">
			...
			@endif
			<button style="padding-left: 30px; padding-right: 30px;" type="button" class="@if($is_mobile) col-xs-6 @endif btn btn-primary waves-effect waves-light @if($pos_settings['disable_draft'] != 0) hide @endif" id="pos-draft" @if(!empty($only_payment)) disabled @endif><i class="fas fa-edit"></i> @lang('sale.draft')</button>
			{{-- <button type="button" class="btn btn-success waves-effect waves-light @if($is_mobile) col-xs-6 @endif" id="pos-quotation" @if(!empty($only_payment)) disabled @endif><i class="fas fa-edit"></i> @lang('lang_v1.quotation')</button> --}}

			@if(empty($pos_settings['disable_suspend']))
				<button style="padding-left: 30px; padding-right: 30px;" type="button" 
				class="@if($is_mobile) col-xs-6 @endif btn btn-warning waves-effect waves-light no-print pos-express-finalize" 
			...
			<button style="padding-left: 30px; padding-right: 30px;" type="button" class="btn bg-navy btn-default @if(!$is_mobile) @endif btn-flat no-print @if($pos_settings['disable_pay_checkout'] != 0) hide @endif @if($is_mobile) col-xs-6 @endif" id="pos-finalize" title="@lang('lang_v1.tooltip_checkout_multi_pay')"><i class="fas fa-money-check-alt" aria-hidden="true"></i> @lang('lang_v1.checkout_multi_pay') </button>

			<button style="padding-left: 30px; padding-right: 30px;" type="button" class="btn btn-success @if(!$is_mobile) @endif btn-flat no-print @if($pos_settings['disable_express_checkout'] != 0 || !array_key_exists('cash', $payment_types)) hide @endif pos-express-finalize @if($is_mobile) col-xs-6 @endif" data-pay_method="cash" title="@lang('tooltip.express_checkout')"> <i class="fas fa-money-bill-alt" aria-hidden="true"></i> @lang('lang_v1.express_checkout_cash')</button>

			@if(empty($edit))
				<button style="padding-left: 30px; padding-right: 30px;" type="button" class="btn btn-danger text-white btn-default btn-flat @if($is_mobile) col-xs-6 @else  @endif" id="pos-cancel"> <i class="fas fa-window-close"></i> @lang('sale.cancel')</button>
			@else
				<button style="padding-left: 30px; padding-right: 30px;" type="button" class="btn btn-danger text-white btn-default btn-flat hide @if($is_mobile) col-xs-6 @else @endif" id="pos-delete" @if(!empty($only_payment)) disabled @endif> <i class="fas fa-trash-alt"></i> @lang('messages.delete')</button>
			@endif
			 
			@if(!$is_mobile)
			<div style="background: #556ee6;" class="pos-total text-black">
			<span style="color:#fff;" class="text">@lang('sale.total_payable')</span>
			<input type="hidden" name="final_total" 
										id="final_total_input" value=0>
			<span id="total_payable" class="number" style="color:#fff;">0</span>
			</div>
			@endif
